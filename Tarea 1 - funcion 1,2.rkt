...
(define (PDC-Sol-aux n tamaño matriz val p-x p-y lim lx-posibles ly-posibles cont flag);auxiliar
  ...
  (cond ((equal? val 1) (PDC-Sol-aux n tamaño (cambiar-valor matriz p-x p-y val) (+ val 1) p-x p-y lim lx-posibles ly-posibles cont flag )) ;verifica el primer movimiento y lo anota en la matriz y aumenta el mov en 1
        ...
        ((equal? flag 1)((print (imprimir matriz))(exit))); verifica si existe solucion, esto se da cuando flag es igual a 1
        ...
        ((< cont 8)    ; verifica si se recorrio todo el lx-posibles y ly-posibles
                ...
                (cond ((< val tamaño) (PDC-Sol-aux n tamaño (cambiar-valor matriz (+ p-x (obt-ele-lista  lx-posibles cont)) (+ p-y (obt-ele-lista ly-posibles cont)) val)
                                       (+ val 1) (+ p-x (obt-ele-lista lx-posibles cont)) (+ p-y (obt-ele-lista  ly-posibles cont)) lim lx-posibles ly-posibles 0 0 )
                                       ; verifica si se han abarcado todos los movimientos tamaño, si no entonces actualiza el nuevo x y, y devuelve el mov aumentado en 1
                                      
                                       (cambiar-valor matriz (+ p-x (obt-ele-lista lx-posibles cont)) (+ p-y (obt-ele-lista ly-posibles cont)) 0)
                                       ;Backtraking en caso de que no se encuentre solucion actualiza la matriz en el punto que se esta  verificando y lo vuelve 0 para devolverse y recalcular
                                       
                                       (PDC-Sol-aux n tamaño matriz  val  p-x   p-y  lim lx-posibles ly-posibles (+ cont 1) 0 )
                                       ; aumenta el contador cont en 1 para acceso al lx-posibles y ly-posibles de movimiento
                                      

                                     )             
                                     
                (else (PDC-Sol-aux n tamaño matriz val 0 0  lim lx-posibles ly-posibles 8 1 )))
                ;si ya se abarco la solucion completa se sale del ciclo de cont y actualiza flag en 1 
                       )
                      (else (PDC-Sol-aux n tamaño matriz  val  p-x   p-y  lim lx-posibles ly-posibles (+ cont 1) 0 ))))
                       ; si no se ha llegado la solucion aumenta el contador cont para abarcar otro movimiento en los lx-posibles, ly-posibles
...
;;funcion principal, verifica que N no sea inferior a 4 y devuelve funcion auxiliar para resolver el problema 
(define(PDC-Todas n posicion)
  (PDC-Sol-aux2 n (* n n) (crear-matriz n) 1 (car posicion) (cadr posicion) (- n 1) '(-1 -2 -2 -1 1 2 2 1) '(-2 -1 1 2 2 1 -1 -2) 0 0))
  

;;funcion auxiliar de la funcion principal, recibe un n, un tamaño (n*n), una matriz llena de ceros, el valor que irá en cada posicion de la matriz, posicion en x, posicion en y, limite de la matriz,
;; lista de posibles movimientos en x, lista de posibles movimientos en y, un contador y un flag que indica cuando se terminó la tarea
(define (PDC-Sol-aux2 n tamaño matriz val p-x p-y lim lx-posibles ly-posibles cont flag);auxiliar
  
  (cond ((equal? val 1) (PDC-Sol-aux2 n tamaño (cambiar-valor matriz p-x p-y val) (+ val 1) p-x p-y lim lx-posibles ly-posibles cont flag )) ;verifica el primer movimiento y lo anota en la matriz y aumenta el mov en 1
        
        ((equal? flag 1)(print (imprimir matriz))); verifica si existe solucion, esto se da cuando flag es igual a 1
        ...
        ((< cont 8)    ; verifica si se recorrio todo el lx-posibles y ly-posibles
                ...
                (cond ((< val tamaño) (PDC-Sol-aux2 n tamaño (cambiar-valor matriz (+ p-x (obt-ele-lista  lx-posibles cont)) (+ p-y (obt-ele-lista ly-posibles cont)) val)
                                       (+ val 1) (+ p-x (obt-ele-lista lx-posibles cont)) (+ p-y (obt-ele-lista  ly-posibles cont)) lim lx-posibles ly-posibles 0 0 )
                                       ; verifica si se han abarcado todos los movimientos tamaño, si no entonces actualiza el nuevo x y, y devuelve el mov aumentado en 1
                                       ...
                                       (PDC-Sol-aux2 n tamaño matriz  val  p-x   p-y  lim lx-posibles ly-posibles (+ cont 1) 0 )
                                       ; aumenta el contador cont en 1 para acceso al lx-posibles y ly-posibles de movimiento
                                      

                                     )             
                                     
                (else (PDC-Sol-aux2 n tamaño matriz val 0 0  lim lx-posibles ly-posibles 8 1 )))
                ;si ya se abarco la solucion completa se sale del ciclo de cont y actualiza flag en 1 
                       )
                      (else (PDC-Sol-aux2 n tamaño matriz  val  p-x   p-y  lim lx-posibles ly-posibles (+ cont 1) 0 ))))
                       ; si no se ha llegado la solucion aumenta el contador cont para abarcar otro movimiento en los lx-posibles, ly-posibles
